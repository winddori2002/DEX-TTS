...
def test(cfg, sample_size):
    ...
    for i, text in enumerate(tqdm(texts)):
        ...
        wav_save_path  = f'{cfg.eval_path}/trg/spk_{spk}_{i}_{emotion}.wav'
...
class Tester:
    ...
    def synthesize(self, sample_size=100):
        ...
        for i, text in enumerate(tqdm(texts)):
            ...
            spk         = item['spk'].to(self.cfg.device)
            
            wav_path    = item['filepath'].replace('/mel/','/trim_wav/').replace('-mel-', '-wav-').replace('.npy', '.wav')
            emotion     = item['emotion']

            with torch.no_grad():
                y_enc, y_dec, attn = self.model(x, x_lengths, n_timesteps=self.cfg.n_timesteps, temperature=1.5, spk=spk)
