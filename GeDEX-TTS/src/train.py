...
class Trainer:
    ...
    def __init__(self, data, cfg):
        ...
        self.cfg       = cfg
        self.model     = GeDEXTTS(cfg.model).to(cfg.device)
    ...
    def train(self):
        ...
        for epoch in range(1, self.cfg.train.epoch+1):
            ...
            self.model.train()
            train_dur_loss, train_diff_loss, train_prior_loss = self._run_epoch(self.train_loader)             
            train_loss = (train_dur_loss + train_diff_loss + train_prior_loss) / 3  

            self.model.eval()
            with torch.no_grad():
                val_dur_loss, val_diff_loss, val_prior_loss = self._run_epoch(self.val_loader, valid=True)
                val_loss  = (val_dur_loss + val_diff_loss + val_prior_loss) / 3  
            ...
            msg  = "Epoch: {:03d} | Step: {:03d} | trn loss: {:.4f} | dur loss: {:.4f} | diff loss: {:.4f} | prior loss: {:.4f}\n".format(epoch, step, 
                                                                                                                                         train_loss, train_dur_loss, 
                                                                                                                                         train_diff_loss, train_prior_loss)
            msg  += "Epoch: {:03d} | Step: {:03d} | val loss: {:.4f} | dur loss: {:.4f} | diff loss: {:.4f} | prior loss: {:.4f}".format(epoch, step, 
                                                                                                                                         val_loss, val_dur_loss, 
                                                                                                                                         val_diff_loss, val_prior_loss)
    ...
    def _run_epoch(self, data_loader, valid=False):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            with autocast(enabled=self.cfg.train.amp):
                dur_loss, prior_loss, diff_loss = self.model.compute_loss(x, x_lengths, y, y_lengths, spk=spk, mask_ratio=curr_mask_ratio, out_size=self.cfg.train.out_size)
        ...
        return total_dur_loss/(i+1), total_diff_loss/(i+1), total_prior_loss/(i+1)
