...
class DiffusionDenoiser(nn.Module):
    def __init__(self, dim, dit_cfg, model_type='dit', dim_mults=(1, 2, 4), groups=8, n_spks=None, spk_emb_dim=64, n_feats=80, pe_scale=1000):
    ...
    def forward(self, x, mask, mu, t, spk=None, mask_ratio=0):
        ...
        t_unet = self.time_pos_emb(t, scale=self.pe_scale)
        t_unet = self.mlp(t_unet)
...
class Diffusion(nn.Module):
    def __init__(self, n_feats, dim, dit_cfg, loss_type='base', precond='edm', model_type='dit', dim_mults=(1, 2), n_spks=1, spk_emb_dim=64, pe_scale=1000):
        super().__init__()
        ...
        self.sampler       = lambda z, mask, mu, spk, steps: ablation_sampler(net=self.precond_model, latents=z, mask=mask, mu=mu, spk=spk, num_steps=steps, solver='euler', discretization='edm', schedule='linear', scaling='none')

        print(f'Model:{model_type}, Precond & Sampler: {precond}')
        
    def forward(self, x, mask, mu, n_timesteps=1, spk=None, infer=False, temperature=1.0, mask_ratio=0):

        if not infer:
            loss = self.loss_fn(self.precond_model, x, mask, mu, spk=spk, mask_ratio=mask_ratio)
            return loss        
        else:
            shape = (mu.shape[0], 80, mu.shape[2])
            x = torch.randn(shape, device=x.device) / temperature + mu
            x = self.sampler(x, mask, mu, spk, n_timesteps)
            return x
