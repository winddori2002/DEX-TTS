...
class EDMLoss(torch.nn.Module):
    ...
    def forward(self, precond_model, x0, mask, mu, ref, ref_lengths, sty, sty_lengths, spk=None, mask_ratio=0):
        ...
        D_yn = precond_model(x0 + n, sigma, mask, mu, ref, ref_lengths, sty, sty_lengths, spk=spk, mask_ratio=mask_ratio)
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, mask, mu, ref, ref_lengths, sty, sty_lengths, spk=None, mask_ratio=0):
        ...
        F_x = self.model((c_in * x), mask, mu, c_noise.flatten(), ref, ref_lengths, sty, sty_lengths, spk=spk, mask_ratio=mask_ratio)
...
def ablation_sampler(net, latents, mask=None, mu=None, ref=None, ref_lengths=None, sty=None, sty_lengths=None, spk=None, class_labels=None, randn_like=torch.randn_like, num_steps=18, sigma_min=None, sigma_max=None, rho=7,
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        denoised = net(x_hat / s(t_hat), sigma(t_hat), mask, mu, ref, ref_lengths, sty, sty_lengths, spk=spk)
        ...
        if solver == 'euler' or i == num_steps - 1:
            ...
            denoised = net(x_prime / s(t_prime), sigma(t_prime), mask, mu, ref, ref_lengths, sty, sty_lengths, spk=spk)
